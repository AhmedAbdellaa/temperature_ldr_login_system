...
static ADC_CHAIN_config_t Global_ADC_Chain_devicses[CHAIN_LENGTH];
...
uint8 TESY_u8init(TESY_CONFIG_t *reference_TESY)
{
	...
	if (reference_TESY != NULL)
	{
		...
		// init LDR
		Global_ADC_Chain_devicses[0].ConversionChannel = (ADC_channel_t)Global_TESY->TE_LDR.AnalogChannel_DigitalSignal;
		Global_ADC_Chain_devicses[0].pvnotificationFunction = &ADC0CallBack;
		Global_ADC_Chain_devicses[0].result = &(Global_TESY->TE_LDR.AnalogChannel_DigitalSignal);
		// init LM
		Global_ADC_Chain_devicses[1].ConversionChannel = (ADC_channel_t)Global_TESY->TE_LM.AnalogChannel_DigitalSignal;
		Global_ADC_Chain_devicses[1].pvnotificationFunction = &ADC7CallBack;
		Global_ADC_Chain_devicses[1].result = &(Global_TESY->TE_LM.AnalogChannel_DigitalSignal);
		//Repeat lm read for solving some hardware issue and remove
		Global_ADC_Chain_devicses[2].ConversionChannel = (ADC_channel_t)Global_TESY->TE_LM.AnalogChannel_DigitalSignal;
		Global_ADC_Chain_devicses[2].pvnotificationFunction = &ADC7CallBack;
		Global_ADC_Chain_devicses[2].result = &(Global_TESY->TE_LM.AnalogChannel_DigitalSignal);
...
void TESY_voidMainSystem(uint8 *ref_u8INTFlag)
{
	...
	// number of leds
	while (*ref_u8INTFlag == 0)
	{

		// read all LDR light level
				ADC_u8StartChainConverstionAsync(Global_ADC_Chain_devicses, CHAIN_LENGTH);
		...
		if (Global_TESY->TE_LM.Temprature != Local_u16OldTemp)
		{
			...
			{
				DCM_voidRotateCW(&Global_TESY->TE_DCMotor);
			}
